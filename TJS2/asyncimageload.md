...
Standard image loading waits for completion when requested, but asynchronous loading allows other processing to occur while the image is loaded in the background.
This makes it possible to design the application so that slow disk reading operations are less noticeable.
...
Loaded images can be copied from the Bitmap to a Layer using Layer.copyFromBitmapToMainImage(Bitmap).
Although called a copy, the data is shared until modified, so it completes instantly.
Since the loading process is asynchronous, the Layer to which the image is passed might already be invalidated by the time loading completes.
When accessing other objects in onLoaded, it is better to check if they have been invalidated.
Alternatively, you must ensure they are not invalidated until onLoaded completes.
...
[Same as the version on GitHub](https://github.com/krkrz/krkrz/blob/master/script/Sample/asyncimageload/startup.tjs)